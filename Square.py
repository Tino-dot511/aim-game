...
class Square():
    ...
    def hits_or_not(self,x,y):
        # Checks if squares have been hit
        ...
        for key,items in dict_copy.items():
            if (x >= items[0] and x <= items[0]+50) and (y >= items[1] and y <= items[1]+50):
                ...
                del self.dict[key] 
                self.check_coordinates() # New square coordinate is generated and checked for overlapping
                rect = self.canvas.create_rectangle(self.lst[-1][0],self.lst[-1][1],self.lst[-1][0]+50,self.lst[-1][1]+50,fill='red')
                # Square is created then added to the dictionary
    ...
    def game_screen(self):
        ...
        self.canvas = self.tk.Canvas(self.root) # Creates the canvas for the game to make the squares that are clicked
